                ...
                <div class="col-12 text-center">
                    <div class="page-header-content">
                        <div class="page-header-content-inner">
                            <h1 style="font-size: 50px;">Каталог</h1>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="row align-items-center katShop">
                        <div class="col-sm-12">
                            <div class="shop-layout-switcher mb-15 mb-sm-0" style="display: flex;justify-content: space-between;align-items: stretch;align-content: center; flex-direction: column;">
                                <input type="text" id="search" placeholder="Наименование" name="search"><br>
    ...
    <script type="text/javascript">
        ...
        function get_cat() {
            $.ajax({
                ...
                success: function(data) {
                    let array = mergeArrayByCategory(data);
                    let main_block = document.querySelector(".col-sm-12.katShop");
                    array.forEach((arr) => {
                        let newDiv = document.createElement("button");
                        newDiv.classList.add("shop_btn");
                        newDiv.textContent = arr["category"];
                        main_block.appendChild(newDiv);
                    });
                    $(".shop_btn").click(function() {
                        console.log(this.innerText);
                        get_product((this.innerText).replace("ids", ""), 0);
                    });
                },
            });
        }

        function mergeArrayByCategory(array) {
            const mergedArray = [];
            const categories = {};
            ...
            array.forEach(item => {
                ...
                if (!categories[category]) {
                    // Если категория еще не добавлена, создаем новую
                    categories[category] = item;
                } else {
                    // Если категория уже есть, объединяем элементы по полю "category"
                    categories[category] = {
                        ...categories[category],
                        ...item
                    };
            ...
            // Преобразование объекта категорий в массив
            for (const category in categories) {
                mergedArray.push(categories[category]);
            }
        ...
        function create_block(bloks) {
            ...
            bloks.forEach((block) => {
                ...
                newDiv.innerHTML = '<div class="product-item"><div class="product-item__thumb"><a ><img class="thumb-primary" src="assets/img/product/' + block["img_600x"] + '.png" alt="Product"><img class="thumb-secondary" src="assets/img/product/600/' + block["img_600x"] + '.png" alt="Product"></a><div class="ratting"><span><i class="ion-android-star"></i></span><span><i class="ion-android-star"></i></span><span><i class="ion-android-star"></i></span><span><i class="ion-android-star"></i></span><span><i class="ion-android-star-half"></i></span></div> </div> <div class="product-item__content"><div class="product-item__info"><h4 class="title"><a >' + block["name"] + '</a></h4><div style="    display: flex;justify-content: space-between;max-width: 500px;"><span class="price"><strong> Гарантия: </strong>' + block["exp"] + ' года</span><span><strong>Цена: </strong>' + block["price"] + ' ₽ </span></div></div><div class="product-item__action"><button class="btn-add-to-cart" onclick="add_product_basket(' + block["id"] + ')">Добавить в корзину</button></div><div class="product-item__desc"><p>' + block["opisanie"] + '</p></div> </div> </div>';
                main_block.appendChild(newDiv);
            });
        }
